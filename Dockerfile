...
ARG LanguageModelContext
ENV LanguageModelContext=${LanguageModelContext}
ARG LanguageModelTemplate
ENV LanguageModelTemplate=${LanguageModelTemplate}

ARG AzureStorageAccName
ENV AzureStorageAccName=${AzureStorageAccName}
ARG AzureBlobContainerName
ENV AzureBlobContainerName=${AzureBlobContainerName}
ARG AzureStorageSasToken
ENV AzureStorageSasToken=${AzureStorageSasToken}

ARG LLMStorageAccName
ENV LLMStorageAccName=${LLMStorageAccName}
ARG LLMStorageContainerName
ENV LLMStorageContainerName=${LLMStorageContainerName}
ARG LLMStorageSasToken
ENV LLMStorageSasToken=${LLMStorageSasToken}
...
RUN pip install -r /app/api/requirements.txt && \
    ...
    cp -r /app/api/talk_to_docs/client/* /var/www/html/
